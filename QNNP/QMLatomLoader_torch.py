...
def Set_center(molecule, set_axis=False):
    cord=torch.array(molecule["coordinates"])[:,1:].astype(float)
    ...
    # (n,n,3)
        ...
        # (n,n)
        bloch_pi=torch.arctan2(bloch_cord[:,:,1],bloch_cord[:,:,0])
        # (n,n)
        min_point=torch.argsort(distance_martix,axis=1)[:,:3]

        theta1=torch.array([bloch_theta[i,x] for i,x in enumerate(min_point[:,1])])
        theta2=torch.array([bloch_theta[i,x] for i,x in enumerate(min_point[:,2])])
        pi1=torch.array([bloch_pi[i,x] for i,x in enumerate(min_point[:,1])])
        pi2=torch.array([bloch_pi[i,x] for i,x in enumerate(min_point[:,2])])
        pi_prime=torch.arctan2(torch.sin(theta2)*torch.sin(pi2-pi1),torch.cos(theta1)*torch.sin(theta2)*torch.cos(pi2-pi1)-torch.sin(theta1)*torch.cos(theta2))
...
def returnangle(qubit):
    the=2*torch.arctan2(torch.abs(qubit[:,:,1]),torch.real(qubit[:,:,0]))
    ph=torch.arctan2(qubit[:,:,1].imag,qubit[:,:,1].real)
...
def Cal_descriptor(cord,distance_matrix,classic=False,new_parameter=None,classic_parameter=None, weigthed=False, atomic_num=None, cutoff_radius=5, halve=False):
    if classic==True:
        descriptor=calqubit(torch.arccos(cord[:,:,2]),torch.arctan2(cord[:,:,1],cord[:,:,0]))*Cutoff_function(distance_matrix[:,:,None],cutoff_radius=cutoff_radius)
        descriptor=descriptor[:,None,:,:]*torch.exp(-classic_parameter[None,:,None,None,0]*torch.pow(distance_matrix[:,None,:,None]-classic_parameter[None,:,None,None,1],2))
        if weigthed==True:
        ...
        descriptor=calqubit(torch.arccos(cord[:,:,2]),torch.arctan2(cord[:,:,1],cord[:,:,0]))
...
def AtomLoader1(sampler=None,idx=None,numb=1,classic=False,new_parameter=None,classic_parameter=None, weigthed=False,cutoff_radius=5, halve=False):
    if sampler==None:
        ...
        return atomloader
    if sampler=='random':
        idx=(torch.random.randint(133885,size=numb)+1).astype(str)
        atomloader={}
        for i,x in enumerate(idx):
...
def AtomLoader2(sampler=None,idx=None,epochs=1,batchs=1,classic=False,new_parameter=None,classic_parameter=None, weigthed=False,cutoff_radius=5, halve=False):
    if sampler==None:
        ...
        return atomloader
    if sampler=='random':
        idx=(torch.random.randint(133885,size=epochs*batchs)+1).astype(str)
        atomloader={}
        k=0
...
def paraLoader(classic_parameter=None, weigthed=False,cutoff_radius=5):
    idx=(torch.random.randint(133885,size=133885)+1).astype(str)
    ...
    def caldot(x,q):
        ...
        for i,y in enumerate(array_split[x]):
            ...
            des=torch.array([torch.sin(descriptor[:,:,0])*torch.cos(descriptor[:,:,1]),torch.sin(descriptor[:,:,0])*torch.sin(descriptor[:,:,1]),torch.cos(descriptor[:,:,0])])
