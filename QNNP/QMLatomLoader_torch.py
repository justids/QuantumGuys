...
def Set_center(molecule, set_axis=False):
    ...
    # (n,n,3)
        ...
        pi_prime=torch.atan2(torch.sin(theta2)*torch.sin(pi2-pi1),torch.cos(theta1)*torch.sin(theta2)*torch.cos(pi2-pi1)-torch.sin(theta1)*torch.cos(theta2))
...
def Cal_descriptor(cord,distance_matrix,classic=False,new_parameter=None,classic_parameter=None, weigthed=False, atomic_num=None, cutoff_radius=5, halve=False):
    if classic==True:
        descriptor=calqubit(torch.arccos(cord[:,:,2]),torch.atan2(cord[:,:,1],cord[:,:,0]))*Cutoff_function(distance_matrix[:,:,None],cutoff_radius=cutoff_radius)
...
def paraLoader(classic_parameter=None, weigthed=False,cutoff_radius=5):
    ...
    def caldot(x,q):
        ...
        for i,y in enumerate(array_split[x]):
            ...
            des=torch.tensor([torch.sin(descriptor[:,:,0])*torch.cos(descriptor[:,:,1]),torch.sin(descriptor[:,:,0])*torch.sin(descriptor[:,:,1]),torch.cos(descriptor[:,:,0])])
